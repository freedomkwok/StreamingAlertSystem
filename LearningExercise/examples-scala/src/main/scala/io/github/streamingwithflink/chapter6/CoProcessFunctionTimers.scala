...
object CoProcessFunctionTimers {
  ...
  def main(args: Array[String]) {
    ...
    val filterSwitches: DataStream[(String, Long)] = env
      .fromCollection(Seq(
        ("sensor_2", 5 * 1000L), // forward readings of sensor_2 for 10 seconds
...
class ReadingFilter
  ...
  override def processElement2(
    ...
    println("register time ", formater.format(ctx.timerService().currentProcessingTime()), "watermark: " ,formater.format(ctx.timerService().currentWatermark()),  formater.format(timerTimestamp) )
  ...
  override def onTimer(
      ...
      out: Collector[SensorReading]): Unit = {
    println("onTimer ", formater.format(ts) )
